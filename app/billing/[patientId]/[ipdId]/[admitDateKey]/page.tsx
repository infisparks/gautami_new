...
export default function BillingPage() {
      ...
      if (billingLoaded && infoLoaded && patientInfoLoaded) {
        ...
        if (infoData) {
          ...
          if (billingData) {
            ...
            // Replace the line `amount: depositTotal,` with the following calculation:
            record = {
              ...record,
              // amount: depositTotal,
              amount: paymentsArray.reduce((sum, p) => sum + (p.type === "advance" ? p.amount : -p.amount), 0),
  ...
  const onSubmitPayment: SubmitHandler<PaymentForm> = async (formData) => {
    ...
    try {
      ...
      const updatedDeposit = updatedPayments.reduce((sum, p) => sum + (p.type === "advance" ? p.amount : -p.amount), 0)
      // The line `await update(recordRef, { totalDeposit: updatedDeposit })` should be removed.
      ...
      // The line updating selectedRecord should now use the calculated updatedDeposit:
