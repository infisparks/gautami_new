...
interface SidebarProps {
  userType: string | null; // e.g., "admin", "staff", "opd", "ipd"
}
...
const Sidebar: React.FC<SidebarProps> = ({ userType }) => {
  ...
  // State to hold the navigation items that are visible to the current user
  const [visibleNavItems, setVisibleNavItems] = useState<NavItemProps[]>([]);

  // Effect to handle mobile view and sidebar collapse state from localStorage
  useEffect(() => {
    const checkMobile = () => {
  ...
  // ** NEW: Effect to filter navigation items based on userType **
  useEffect(() => {
    ...
    const allNavItems: NavItemProps[] = [
        ...
        },
        {
            ...
            submenu: [
                ...
                { title: "Casulity Management", icon: <FileText size={20} />, href: "/casulity/list" },
            ],
        },
        {
            title: "Mortality",
    ...
    switch (userType) {
      case "admin":
        ...
        break;
      case "staff":
        // Staff see all items except Dashboard and Admin management
        setVisibleNavItems(
          allNavItems.filter(
  ...
  // Renders the menu and submenus from the filtered list
  const renderNavItems = (items: NavItemProps[]) => {
    // ... (The entire renderNavItems function remains unchanged)
        ...
        return (
            <div key={item.title} className="mb-1">
                {hasSubmenu ? (
                  ...
                  // ... (Code for non-submenu items remains the same)
                    <TooltipProvider delayDuration={350}>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Link href={item.href || "#"}>
  ...
  return (
    <div className="flex">
        ...
        >
            ...
            {/* RENDER THE VISIBLE (FILTERED) NAVIGATION ITEMS */}
            <nav className={cn(
            ...
            <div className={cn(
              ...
              {/* ... (Logout Button code remains unchanged) ... */}
                <TooltipProvider delayDuration={350}>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <button
